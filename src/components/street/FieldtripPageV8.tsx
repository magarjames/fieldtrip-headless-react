...
export function FieldtripPageV8({
  theme = "bright",
  backdrop,
  ...
  useState(() => {
    // We only run this if the Shopify keys are actually configured in .env
  ...
  const addToCart = (variantId?: string) => {
    ...
    setCart(prev => {
      ...
      return [...prev, { id: variantId, variantId, quantity: 1 }];
    });
  };
  ...
  return (
    ...
    >
      ...
      {backdrop && (
        <div aria-hidden="true" className="ft-backdrop">
          {backdrop}
        </div>
      )}
      ...
      <header className="shell flex items-center gap-4 py-4">
        ...
        <div className="absolute right-4 top-4 z-50 sm:right-6 lg:right-8">
          {onShopClick ? (
            ...
            >
              {isCheckingOut ? "Loading..." : `Checkout (${cart.reduce((a, b) => a + b.quantity, 0)})`}
            </button>
          ) : (
            <a
              href={shopHref}
                    ...
                    {f.pieces.map((pid) => {
                      ...
                      return (
                        <li key={pid}>
                          <button
                            type="button"
                            onClick={(e) => { e.preventDefault(); addToCart((p as any).shopifyVariantId); }}
                            className="lbl inline-flex min-h-11 items-center rounded-full border px-3 transition-colors hover:bg-black hover:text-white"
                            style={{ borderColor: "var(--hair)" }}
                          >
                            {(p as any).shopifyVariantId ? `Add ${p.name}` : p.name} · {money(p.price)}
                          </button>
                        </li>
                      );
                    })}
            ...
            <div className="mt-10 grid grid-cols-2 gap-x-3 gap-y-10 md:grid-cols-3 lg:grid-cols-4">
              {shown.map((p) => (
                <article key={p.id} className="card rv" style={{ ["--hue" as string]: p.hue }}>
                  <div 
                    onClick={() => addToCart((p as any).shopifyVariantId)} 
                    className={`block ${((p as any).shopifyVariantId) ? 'cursor-pointer' : ''}`}
                  >
                    <div
                    ...
                    <div className="mt-3 flex items-baseline justify-between gap-3">
                      <h3 className="text-[1rem]">
                        {p.name}
                        {(p as any).shopifyVariantId && (
                           <span className="ml-2 text-xs font-bold text-[var(--pop)] bg-[var(--ink)] px-2 py-0.5 rounded-full">+ ADD TO CART</span>
                        )}
                      </h3>
